...
<div class="content">
<%
  if(messageList.size()>0&&unReadNum>0){
%>
<p style="margin-top: 30px;margin-left: 100px;font-size: 30px;color: rgb(0,27,85);">未读邮件</p>
<table border="1" cellspacing="0" >
  ...
  <tr>
    ...
    <td width="165px" height="40px" >&nbsp;&nbsp;<input type="button" value="删  除" onclick="javascrtpt:window.location.href='doDelete.jsp?msgid=<%=message.getId()%>'"
      ...
      <input type="button" value="设为已读" onclick="javascrtpt:window.location.href='doRead.jsp?msgid=<%=message.getId()%>&pageNum=<%=pageNum%>'"
             style="width: 63px;height:25px;"/> </td>
  </tr>
  <%
    }
    }
  ...
  <input type="button" value="返回首页" onclick="javascrtpt:window.location.href='firstpage.jsp'" style="width: 70px;height: 30px;margin-top: 30px;margin-left: 100px;"/>
  <input type="button" value="进入收件箱" onclick="javascrtpt:window.location.href='listMsg.jsp'" style="width: 75px;height: 30px;margin-top: 30px;margin-left: 100px;"/><br/>
  <%
}else {
%>
  ...
  <img src="image/nounread.png" style="margin: 50px 0 0 400px "/>
<%
  }
%>
</div>
